...
export type UpProtocol = 'https' | 'http'
...
export interface Config extends Partial<Omit<InternalConfig, 'upprotocol' | 'uphost'>> {
  /** 上传域名协议 */
  upprotocol?: UpProtocol | 'https:' | 'http:'
  /** 自定义上传域名 */
  ...
  public async putFile(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof QiniuRequestError) {
        const reqId = this.aborted ? '' : error.reqId
        const code = this.aborted ? -2 : error.code
        ...
        const needRetry = !this.aborted && RETRY_CODE_LIST.includes(error.code)
